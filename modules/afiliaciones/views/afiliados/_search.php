...
<div id="search-afiliados" class="afiliados-search">
    ...
    <div class="form-group">
        <?php echo Html::button('<i class="fa fa-search"> Buscar</i>', [ 'class' => 'btn btn-success', 'id' => 'btn-search-afiliados', 'title' => 'Buscar' ]); ?>						
		<?php echo Html::button('<i class="fa fa-ban"> Cancelar</i>', [ 'class' => 'btn btn-danger', 'onClick' => '$("#div-search-afiliados").hide(); $("#div-index-afiliados").show();', 'title' => 'Cancelar' ]); ?>
    </div>
...
<?php
$this->registerJs(' 
 $("#btn-search-afiliados").on("click", function(){ 
   var url = "'.Yii::$app->urlManager->createUrl(['/afiliaciones/afiliados/search']).'";
   var form = $("#form-search-afiliados");
   var form_afiliados = $("#form-create-afiliados");
	 ...
	 url: url,
		...
		success:function(data){
			if(data != "")
		    {
				...
				if(parsed!=null){
					if(parsed.info == "true"){
					alert("AFILIADO EXISTENTE:\n--------------------------\n\nEl número de identificación ingresado pertenece a una persona que ya esta registrada como afiliado en la base de datos.");							
					}else if(parsed.info == "false"){
						$("#div-search-afiliados").hide();
						form_afiliados[0].reset();				
						$("#div-create-afiliados").show();
